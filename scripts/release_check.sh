...
run_preflight() {
  ...
  capture_step pytest-0 env PYTHONPATH="$ROOT/src" PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 \
    "$PYTHON_BIN" -m pytest -q \
    tests/test_collision_spacing.py tests/test_damage.py tests/test_generational.py tests/test_integrity.py tests/test_outputs.py \
    -k "$filter"
  capture_step pytest-1 env PYTHONPATH="$ROOT/src" PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 \
    "$PYTHON_BIN" -m pytest -q \
    tests/test_packaging.py tests/test_receipts.py tests/test_simulation.py \
    -k "$filter"
  capture_step pytest-2 env PYTHONPATH="$ROOT/src" PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 \
    "$PYTHON_BIN" -m pytest -q \
  ...
  capture_step clean_copy "$PYTHON_BIN" -c \
    'import shutil,sys; from pathlib import Path; root=Path(sys.argv[1]); clean=Path(sys.argv[2]); shutil.copytree(root,clean,ignore=shutil.ignore_patterns(".git",".pytest_cache","__pycache__","*.egg-info",".release_preflight.json",".release_preflight_parts",".release_semantic.json",".state_restoration_semantic",".state_restoration_work","*.zip","*.sha256"))' \
    "$ROOT" "$clean"
...
for shard in 0 1 2 3 4 5 6 7; do
  PYTHONPATH="$ROOT/src" "$PYTHON_BIN" -u scripts/release_check.py --semantic-shard "$shard"
done
PYTHONPATH="$ROOT/src" "$PYTHON_BIN" -u scripts/release_check.py --semantic-finalize
